...
//		Set<AirConditionDemand> airConditionDemands = socialActivityToDemandDAO.findById(3).getAirConditionDemands();
	...
	public RealTimeDecision createRealTimeDecision(RealTimeDecision realTimeDecision,int realTimeDemandId) throws Exception{
		emh.beginTransaction();	
		EntityManager em = emh.getEntityManager();
		RealTimeDecision rtd = realTimeDecision;
		
		
		RealTimeDemand realTimeDemand = em.getReference(RealTimeDemand.class, realTimeDemandId);
		realTimeDecision.setRealTimeDemand(realTimeDemand);
		em.persist(realTimeDemand);
